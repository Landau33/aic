...
class NICPortEstimator(Node):
    def __init__(self):
        ...
        # 外参（camera → world）
        self.T_left_to_world = np.array(
            [
                [0.500, 0.837, 0.224, -0.101],
        ...
        self.T_right_to_world = np.array(
            [
                [0.500, -0.837, -0.224, 0.101],
                [0.837, 0.533, -0.125, 0.056],
                [0.224, -0.125, 0.967, 0.015],
        ...
        self.recognized_pub = self.create_publisher(
            Image, "/center_camera/image/recognized", 10
        )
        ...
        ts = message_filters.ApproximateTimeSynchronizer(
            [left_sub, center_sub, right_sub], queue_size=10, slop=0.1
        )
        ts.registerCallback(self.image_callback)

        self.get_logger().info(
            "NIC Port Estimator started. Publishing recognized image to /center_camera/image/recognized"
        )
